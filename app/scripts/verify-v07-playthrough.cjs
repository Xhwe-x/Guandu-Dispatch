...
function button(label, within = document) {
  const matches = [...within.querySelectorAll('button')].filter((node) => text(node).includes(label));
...
act(() => {
  root.render(React.createElement(GameProvider, { initialState: state, storage: new MemoryStorage() }, React.createElement(GameShell)));
});
...
(async () => {
  ...
  console.log('v0.7 full UI playthrough passed (title → investigation → interrogation → deduction → bait → report → ending).');
  act(() => root.unmount());
})().catch((error) => {
  console.error(error);
  process.exitCode = 1;
